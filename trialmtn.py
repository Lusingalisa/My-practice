...
def sendMoney():
    ...
    phoneNumber=input("Enter number you are sending to")
    amount = int(input("Enter the amount you are sending"))
    ConfirmSend(amount,phoneNumber)
    mainMenu()

def ConfirmSend(amount, number):
    pin = 12345
    print(f"You are sending {amount} to {number}\n Please Enter your Mobile Money Pin")
    enteredpin = int(input())
    
    if (enteredpin == pin):
        print(f"The money is sent to {number}")
    else:
